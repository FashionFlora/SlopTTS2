...
def main(config_path):
    ...
    accelerator = Accelerator(project_config=configAcc,
                              split_batches=True,
    ...
    if not load_pretrained:
        if config.get('first_stage_path', '') != '':
            ...
            model, _, start_epoch, iters = load_checkpoint(model, 
                ...
                ignore_modules=['bert', 'bert_encoder', 'predictor', 'diffusion']) # keep starting epoch for tensorboard log
            ...
            accelerator.print("Copied style_encoder -> predictor_encoder (no deepcopy).")
        else:
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for i, batch in enumerate(train_dataloader):
            ...
            # denoiser training
            if epoch >= diff_epoch:
                # (optional) estimate sigma_data robustly if you're using KDiffusion with estimation
                if model_params.diffusion.dist.estimate_sigma_data:
                    # robust per-batch estimate + running EMA outside this snippet if needed
                    model.diffusion.sigma_data = s_trg.std(axis=-1).mean().item()
                    running_std.append(model.diffusion.sigma_data)
                # diffusion loss (no sampling-in-the-loop L1)
                loss_diff, aux = model.diffusion.diffusion(
                    s_trg.unsqueeze(1),
                    embedding=bert_dur,
                    return_aux=True,
                )
            ...
            y_rec, mag , phase = model.decoder(en, F0_fake, N_fake, s)
            ...
            loss_mag_phase = mag_phase_loss(mag ,phase,  wav.squeeze(1)) 
            if start_ds:
            ...
            if epoch >= diff_epoch and (iters % 30 == 0):
                with torch.no_grad():
                    s_pred_quick = sampler(
                        torch.randn_like(s_trg).unsqueeze(1),
                        embedding=bert_dur,
                        num_steps=6,                # szybki, tani
                        embedding_scale=1.0,
                        embedding_mask_proba=0.0
                    ).squeeze(1)
                loss_sample_anchor = F.l1_loss(s_pred_quick, s_trg)
                g_loss = g_loss + 0.05 * loss_sample_anchor  # bardzo mała waga
            ...
            if (i+1)%log_interval == 0:
                logger.info ('Epoch [%d/%d], Step [%d/%d], Loss: %.5f, Disc Loss: %.5f, Dur Loss: %.5f, CE Loss: %.5f, Norm Loss: %.5f, F0 Loss: %.5f, LM Loss: %.5f, Gen Loss: %.5f, Sty Loss: %.5f, Diff Loss: %.5f, DiscLM Loss: %.5f, GenLM Loss: %.5f, MagPhase Loss: %.5f '
                    %(epoch+1, epochs, i+1, len(train_list)//batch_size, running_loss / log_interval, d_loss, loss_dur, loss_ce, loss_norm_rec, loss_F0_rec, loss_lm, loss_gen_all, loss_sty, loss_diff, d_loss_slm, loss_gen_lm , loss_mag_phase))
