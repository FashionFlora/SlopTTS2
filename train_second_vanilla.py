...
def main(config_path):
    ...
    if not load_pretrained:
        if config.get('first_stage_path', '') != '':
            ...
            epochs += start_epoch
            #print(torch.isnan(model.style_encoder.some_layer.weight).any())
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for i, batch in enumerate(train_dataloader):
            ...
            if epoch >= diff_epoch:
                with torch.no_grad():
                    num_steps = torch.randint(16, 25, (1,)).item()  
                if model_params.diffusion.dist.estimate_sigma_data:
                    model.diffusion.sigma_data = s_trg.std(axis=-1).mean().item() # batch-wise std estimation
                ...
                loss_sty = F.l1_loss(s_preds, s_trg.detach())# EDM loss
