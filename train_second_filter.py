...
def main(config_path):
    ...
    start_epoch = 0
    iters = 0
    window_min_dur_loss = float('inf')
    window_max_dur_loss = float('-inf')
    window_min_dur_path = None
    window_max_dur_path = None
    ...
    filtered_train_path = train_path + ".filtered.txt"
    filtered_backup_dir = osp.join(log_dir, "filtered_backups")
    os.makedirs(filtered_backup_dir, exist_ok=True)

    # If a filtered copy doesn't exist yet, create it from the original
    if not osp.exists(filtered_train_path):
        shutil.copy(train_path, filtered_train_path)
        logger.info("Created filtered copy: %s", filtered_train_path)

    # filtered_counter for naming backups (keeps only in-memory; optional persistence)
    filtered_counter = 0

    # Build dataloaders using the original train list (unchanged)
    for epoch in range(start_epoch, epochs):
        ...
        for i, batch in enumerate(train_dataloader):
            ...
            if len(dur_losses) > 0:
                batch_avg_dur = float(np.mean(dur_losses))
                any_bad = any(l > (batch_avg_dur + 0.4) for l in dur_losses)
                if any_bad:
                    try:
                        # indices and paths considered bad in this batch
                        bad_indices = [idx for idx, l in enumerate(dur_losses)
                                       if l > (batch_avg_dur + 0.4)]
                        bad_paths = [paths[idx] for idx in bad_indices]

                        bad_paths_norm = {os.path.normpath(p) for p in bad_paths}
                        bad_basenames = {os.path.basename(p) for p in bad_paths}

                        # Read the current filtered copy (cumulative)
                        with open(filtered_train_path, "r", encoding="utf-8") as f:
                        ...
                        for ln in lines:
                            ...
                            left_basename = os.path.basename(left)
                            if (left_norm in bad_paths_norm) or (left_basename in bad_basenames):
                                removed_lines.append(left)
                            else:
                                new_lines.append(ln)

                        if len(removed_lines) > 0:
                            # Atomic overwrite of filtered file
                            tmp_path = filtered_train_path + ".tmp"
                            with open(tmp_path, "w", encoding="utf-8") as f:
                                for nl in new_lines:
                                    f.write(nl + "\n")
                            os.replace(tmp_path, filtered_train_path)

                            # Save a timestamped backup of the filtered state (optional)
                            ts = int(time.time())
                            filtered_counter += 1
                            backup_name = f"train.filtered.v{filtered_counter}.{ts}.txt"
                            backup_path = osp.join(filtered_backup_dir, backup_name)
                            with open(backup_path, "w", encoding="utf-8") as f:
                                for nl in new_lines:
                                    f.write(nl + "\n")

                            logger.info(
                                "Updated filtered copy %s (removed %d entries). Backup: %s. Examples: %s",
                                filtered_train_path, len(removed_lines), backup_path,
                                ", ".join(list(removed_lines)[:10])
                            )
                        else:
                            logger.debug("Detected bad paths in batch, but none matched current filtered list entries.")
                    except Exception:
                        logger.exception("Failed to update/load the cumulative filtered train list:")
            for idx, loss in enumerate(dur_losses):
                if loss < window_min_dur_loss:
                    window_min_dur_loss = loss
                    window_min_dur_path = paths[idx]
                if loss > window_max_dur_loss:
                    window_max_dur_loss = loss
                    window_max_dur_path = paths[idx]
            ...
            if iters % 100 == 0:
                min_dir = os.path.join(log_dir, "min")
                max_dir = os.path.join(log_dir, "max")
                os.makedirs(min_dir, exist_ok=True)
                os.makedirs(max_dir, exist_ok=True)

                dst_min = os.path.join(min_dir, f"{iters}_"+os.path.basename(window_min_dur_path))
                dst_max = os.path.join(max_dir, f"{iters}_"+os.path.basename(window_max_dur_path))
                shutil.copy2(window_min_dur_path, dst_min)
                shutil.copy2(window_max_dur_path, dst_max)
                print(f"Copied window min dur loss file {window_min_dur_path} to {dst_min} (min dur loss: {window_min_dur_loss:.4f})")
                print(f"Copied window max dur loss file {window_max_dur_path} to {dst_max} (max dur loss: {window_max_dur_loss:.4f})")
