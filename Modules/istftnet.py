...
class Generator(torch.nn.Module):
    def __init__(
        ...
        for i, (u, k) in enumerate(
            ...
            self.ups.append(
                UpsamplePixelShuffle1D(in_ch, out_ch, kernel_size=k, r=u)
            )

        n_bins = gen_istft_n_fft // 2 + 1
        ...
        for i in range(len(self.ups)):
            ...
            if i + 1 < len(upsample_rates):  #
                ...
                self.noise_convs.append(Conv1d(n_bins * 2, c_cur, kernel_size=1))
                self.noise_res.append(
                    resblock(c_cur, 11, [1, 3, 5], style_dim)
                )

        self.post_n_fft = gen_istft_n_fft
        self.conv_post = weight_norm(Conv1d(ch, n_bins * 2, 7, 1, padding=3))
    ...
    def forward(self, x, s, f0):
        """
        Accepts an optional 'target_len' to ensure the output waveform
        matches the length of the ground-truth audio.
        """
        with torch.no_grad():
        ...
        n_bins = self.post_n_fft // 2 + 1
        spec = torch.exp(x[:, :n_bins, :])
        phase = torch.sin(x[:, n_bins:, :])
        #phase = phase + 0.05 * torch.randn_like(phase) * (self.phase_scale / math.pi)
        # FIX: Pass the target_len to the iSTFT to ensure correct output length
...
class Decoder(nn.Module):
    def __init__(self, dim_in=512, F0_channel=512, style_dim=64, dim_out=80, 
                ...
                upsample_rates = [8, 8], # New rates: 8 * 8 * 8 = 512
                upsample_initial_channel=512,
                resblock_dilation_sizes=[[1,3,5], [1,3,5], [1,3,5]],
                upsample_kernel_sizes=[16, 16], # Kernels matching the new rates
                gen_istft_n_fft=64, # Increased for better quality
                gen_istft_hop_size=8): # New iSTFT hop size
        super().__init__()
    ...
    def forward(self, asr, F0_curve, N, s):
        ...
        if self.training:
            # Note: The following block for downsampling F0_curve will now
            # operate on a tensor of zeros, resulting in zeros.
            # This is likely the intended behavior for this modification.
            downlist = [0, 3, 7]
